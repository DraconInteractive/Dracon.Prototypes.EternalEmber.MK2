	...
	public class UIItemSlot : UISlotBase, IUIItemSlot
    {
		...
		public static void PrepareTooltip(UIItemInfo itemInfo)
        {
			...
			else if (itemInfo.ItemType == 2)
            {
				...
				UITooltip.AddLineColumn(itemInfo.Armor.ToString() + "", "ItemAttribute");
				UITooltip.AddLineColumn(itemInfo.Block.ToString() + "", "ItemAttribute");
			}
...
//            UITooltip.AddLine("+" + itemInfo.Strength.ToString() + " Strength", "ItemStat");
			...
			foreach (KeyValuePair<string,float> attachStat in itemInfo.ItemAttributes) {
				UITooltip.AddLine ("+" + attachStat.Value.ToString () + " " + attachStat.Key, "ItemStat");
			}
            UITooltip.AddSpacer();
