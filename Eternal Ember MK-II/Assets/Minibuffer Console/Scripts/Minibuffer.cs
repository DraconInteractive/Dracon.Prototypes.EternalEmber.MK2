...
public class Minibuffer : MonoBehaviour {
  ...
  */
  ...
  public struct AdvancedOptions {
    ...
    public bool showKeyBindingHint;
  ...
  private KeyCode lastKeyCode; // XXX currentKeyCode? No.
  [NonSerialized]
  ...
  */
  ...
  private void AddKeyBindings(CommandInfo ci, bool overwriteDups = false) {
    ...
    foreach(var keyBinding in c.keyBindings) {
      if (! overwriteDups && keymap.ContainsKey(keyBinding) && keymap[keyBinding] == ci.name) {
  ...
  */
  ...
  internal IPromise<T> Read<T>(PromptInfo prompt) {
    ...
    return interpreter.FillType(prompt, typeof(T))
      .Then(obj => {
  ...
  */
  private IPromise<PromptResult> MinibufferEdit(PromptInfo prompt,
    ...
    if (editing) {
      ...
      return Promise<PromptResult>
        .Rejected(new MinibufferException("Minibuffer already in use!"));
    }
    ...
    editState.history = history;
    if (editState.history != null) {
      editState.prevHistory = history.Reverse<string>().ToQueue<string>();
    } else {
      editState.prevHistory = new Queue<string>();
    }
    if (editState.nextHistory == null)
      editState.nextHistory = new Queue<string>();
    editState.nextHistory.Clear();
  ...
  private void ActivateInput() {
    ...
    // print("setg selectionfocusposition");
  ...
  */
  ...
  /**
     Look at the next key but let %Minibuffer still
     handle it.
  */
  ...
  */
  ...
  /**
     ...
     insensitive).
  */
  public IPromise<char> ReadValidChar(List<char> valid,
                                      string msg) {
    if (valid.Count == 0) {
  ...
  */
  ...
  public void ExecuteExtendedCommand(string completer = "command") {
    ...
    Read(passingPrefixArg == null
      ...
      .Then(cmd =>  // when finished
          {
            currentPrefixArg = passingPrefixArg;
            if (cmd != null) {
              var bindings = KeyBindingsForCommand(cmd);
              if (bindings.Any() && advancedOptions.showKeyBindingHint)
  ...
  internal void MinibufferExit(bool abort) {
    if (! editing) {
      Debug.LogWarning("MinibufferExit called outside while 'editing' = false.");
      return;
    }
    ...
    editState.prevHistory.Enqueue(inputCopy);
    if (editState.history != null) {
      editState.history.Clear();
      editState.history
        .AddRange(editState.prevHistory.Concat(editState.nextHistory));
    ...
    object result = null;
    try {
      if (oldPrompt.completerEntity.coercer != null) {
        Assert.IsNotNull(oldPrompt.desiredType);
        result = oldPrompt.completerEntity.coercer.Coerce(inputCopy,
  ...
  private bool CompleterHasMatch(string input) {
  ...
  private bool CanCoerce(string input) {
    try {
  ...
  public void TabComplete() {
    ...
    //print("tab complete on " + inputCopy);
    if (editState.prompt.completerEntity.completer != null) {
      ICompleter completer = editState.prompt.completerEntity.completer;
      var matches = completer.Complete(inputCopy).ToList();
      matches.Sort();
      if (matches.Count > 4) {
      ...
      if (matches.Count == 1) {
        ...
        gui.input.MoveTextEnd(false); // move-end-of-line
        gui.input.Deselect();
        ...
        int matchLength = Math.Max(0, inputCopy.Length);
        //var minLength = matches.Aggregate(0, (accum, str) => Math.Min(accum, str.Length));
        var maxLength = matches
        ...
        IEnumerable<string> annotatedMatches;
        //float windowLength;
        string annotatedFormat = null;
        if (showAnnotations && annotater != null) {
          var annotations = matches.Select(v => annotater.Annotate(v));
          var annotatedMaxLength
            = annotations.Aggregate(0, (accum, str) => Math.Max(accum, str.Length));
          annotatedFormat = "{0,-" + maxLength + "} {1,-" + annotatedMaxLength + "}";
          annotatedMatches = matches
            .Select(v => string.Format(annotatedFormat, v, annotater.Annotate(v)));
          maxLength += annotatedMaxLength;
        } else {
          annotatedMatches = matches;
        }
        ...
        if (showGroups && grouper != null) {
          annotatedMatches = matches
            ...
            .SelectMany(g =>
                        g.OrderBy(x => x)
                        .Select(x => showAnnotations && annotater != null && annotatedFormat != null
                                ? " " + annotatedFormat.Formatted(x, annotater.Annotate(x))
                                : "  {0}".Formatted(x))
                        .Prepend(g.Key));
          ...
          editState.candidates = matches.ToList();
        }
        gui.autocomplete.sizeInChars
          = new Vector2(maxLength + 1,
                        Mathf.Min(advancedOptions.maxAutocompleteLines, matches.Count));
        //var size = autocomplete.sizeInChars;

        //autocomplete.content.text

        gui.autocompleteField.text
          = String.Join("\n",
                        annotatedMatches
                        .ToArray());
        ...
        if (matchLength != 0) {
          for (;;) {
            if (matchLength >= matches[0].Length)
              break;
            char c = char.ToLower(matches[0][matchLength]);
            for (int i = 1; i < matches.Count; i++) {
              if (matchLength >= matches[i].Length
                  || char.ToLower(matches[i][matchLength]) != c)
                goto endOfLoop;
            }
            matchLength++;
          }
      ...
      endOfLoop:
        var longestMatch = matches[0].Substring(0, Math.Min(matchLength, matches[0].Length));
        var caseInsensitive = true;
        if (longestMatch.StartsWith(inputCopy, caseInsensitive, null))
          this.input = longestMatch;
        if (matches.Contains(this.input))
          MessageInline(" [Complete but not unique]");
        //lastTabCompleteInput = inputCopy;
        gui.input.MoveTextEnd(false); // move-end-of-line
        gui.input.Deselect();
      }
...
//         gui.autocomplete.ScrollDown();
  ...
  */
  ...
  [Command("self-insert-command",
           ...
           group = "editing",
           tags = new [] { "pass-to-inputfield", "no-repeat" })]
  /**
  ...
  [Command("universal-argument",
           description = "Pass a numerical or boolean argument to next command.",
           group = "core",
           keyBinding = "C-u",
           tag = "no-repeat")]
